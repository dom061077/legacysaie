			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="denominacion" title="${message(code: 'carrera.denominacion.label', default: 'Denominacion')}" />
						...
						<g:sortableColumn property="perfilegresado" title="${message(code: 'carrera.perfilegresado.label', default: 'Perfilegresado')}" />
				...
				<g:each in="${carreraInstanceList}" status="i" var="carreraInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${carreraInstance.id}">${fieldValue(bean: carreraInstance, field: "denominacion")}</g:link></td>
					
						<td>${fieldValue(bean: carreraInstance, field: "duracion")}</td>
